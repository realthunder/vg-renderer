...
void polylineStroke(Stroker* stroker, Mesh* mesh, const Vec2Stored* vtx, uint32_t numPathVertices, float strokeWidth)
{
	...
	const uint32_t numPointsHalfCircle = bx::max<uint32_t>(2u, (uint32_t)bx::ceil(bx::kPi / da));
		...
		if (leftPointProjDist >= 0.0f) {
			...
			if (_LineJoin == LineJoin::Miter) {
				...
				if (_LineJoin == LineJoin::Round) {
					...
					numArcPoints = bx::max<uint32_t>(2u, (uint32_t)((a12 - a01) / da));
			...
			if (_LineJoin == LineJoin::Miter) {
				...
				if (_LineJoin == LineJoin::Round) {
					...
					numArcPoints = bx::max<uint32_t>(2u, (uint32_t)((a01 - a12) / da));
					arcDa = ((a12 - a01) / (float)numArcPoints);
				}
